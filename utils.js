// transforms string in number and replace comma with the point to perform current calculation
export function getNumberByStr(str) {
  return parseFloat(str.replace(',', '.'));
}

// replace comma with point to display comma on the screen
export function fromNumberToResult(result) {
  return String(result).replace('.', ',');
}

// current operation manager
